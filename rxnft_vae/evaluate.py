...
class Evaluator(nn.Module):
    ...
    def generate_discrete_latent(self, latent_size, method="gumbel", temp=0.4):
        """
        Generate latent variables conforming to discrete distribution
        """
	logits = torch.zeros(1, latent_size, 2)  # logits size for binary case
	gumbel_noise = -torch.log(-torch.log(torch.rand(logits.shape) + 1e-20) + 1e-20)
